...
_SCRIPT_TEXT = {
    ...
    "cfo_fin": (
        ...
        "Turning to guidance. For the third quarter, we expect net revenue of $26.4 to $26.9 million. For "
        "the full year, we are raising our guidance to $104 to $106 million in net revenue, from $103 to "
        "$105 million; raising adjusted earnings per share to $0.53 to $0.56, from $0.52 to $0.55; and we "
        "continue to expect an adjusted EBITDA margin of approximately 21%. The raise reflects the "
        "first-half beat now banked into the year, together with the back-half-weighted go-live cadence.\n\n"
        "I want to be direct about the shape of the second half, building on Marcus's point, because I know "
        "it will come up in your models. Our prior-year second half included approximately $3 million of "
        "revenue from a government payments program that is now unwinding on its planned schedule. That "
        "makes our reported year-over-year growth optically lower in the third and fourth quarters. On a "
        "comparable basis — excluding that program from the prior-year base — our implied second-half growth "
        "continues in the high teens to around twenty percent, in line with what we delivered in the first "
        "half. So the deceleration you may see in the headline year-over-year number is a comparison effect, "
        "not a change in the underlying trajectory of the business.\n\n"
        "A couple of assumptions embedded in our guidance, since I know you will ask. We assume no change in "
        "the interest-rate environment and no contribution from growth in prepaid float — on that point, "
        "float balances were stable in the quarter, the contribution to revenue was consistent with prior "
        "periods, and to be very clear, we do not model float as a growth driver; it is a small and stable "
        "contributor. We assume the back-half-weighted go-live cadence Tom described, which is why we expect "
        "the second half to be sequentially stronger than the first. And we assume continued low-single-digit "
        "growth in the legacy book, with the integrated book carrying the story. If the go-lives land as we "
        "expect, that is where the upside bias to the range comes from.\n\n"
        "To summarize: revenue grew 18% and beat our guidance, the mix shifted further toward integrated and "
        "lifted take-rate, margins expanded on both gross margin and operating leverage, we generated "
        "positive free cash flow, and we raised the full-year outlook on both revenue and earnings. The "
        "financial model is doing exactly what the operating model is built to make it do.\n\n"
