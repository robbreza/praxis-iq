...
- **9 · Script generation (finale):** "And it doesn't stop at roadshows. When earnings come, the script drafts itself — every section, from the CEO's remarks to the CFO's, generated by AI and shown side-by-side with the live analytics. The whole team works from one source, so the script comes together in a fraction of the time." *(Earnings Cycle → Script Generation → Script Canvas — the RAISED guidance decision + section nav + AI trend-read shown side-by-side.)*
